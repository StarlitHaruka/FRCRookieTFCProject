...
public class SwerveModule implements Logged, AutoCloseable {
  ...
  public void updateSetpoint(SwerveModuleState setpoint, ControlMode mode, double movementRatio) {
    ...
    setpoint.optimize(hardware.rotation());
    ...
    double driveTVolts =
        switch (mode) {
          case CLOSED_LOOP_VELOCITY ->
              driveTranslationFeedforward.calculateWithVelocities(
                  this.setpoint.speedMetersPerSecond, setpoint.speedMetersPerSecond);
    ...
    double driveRVolts =
        switch (mode) {
          case CLOSED_LOOP_VELOCITY ->
              driveRotationFeedforward.calculateWithVelocities(
                  this.setpoint.speedMetersPerSecond, setpoint.speedMetersPerSecond);
          case OPEN_LOOP_VELOCITY ->
